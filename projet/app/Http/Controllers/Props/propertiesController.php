...
public function search(Request $request)
{
    ...
    $offerType = $request->input('type');
    $city = $request->input('city');

    // Effectuer la recherche selon les critères
    $properties = Property::where('type', $offerType)
...
{
    $properties = Property::where('type', 'Rent')->get();  // Remplace 'Rent' par le bon filtre si nécessaire.
    return view('home', compact('properties'));
}
    ...
    public function single($id)
    {
        ...
        $relatedProps = Property::where('home_type_id', $singleProp->home_type_id)
    ->where('id', '!=', $id)
    ->take(3)
